...
export const getSnapWall = (walls: Array<IWall>, position: Vector3, snapProp?: ISnap, distance = 0.2) => {
    const snap: ISnap = {
        snapStart: snapProp?.snapStart || null,
        snapEnd: snapProp?.snapEnd || null,
    };
...
export const getDirectionString = (start?: Vector3 | null, end?: Vector3 | null, dir?: Vector3) => {
    let direction = dir;
    if (start && end) {
        direction = new Vector3().subVectors(end, start).normalize();
    }
